...
function copyDirRecursiveSync(src, dest) {
  ...
  if (!exists) {
    throw new Error(`Diretório de origem não existe: ${src}`);
  }
...
console.log('Iniciando script de build...');
...
try {
  ...
  console.log("Copiando 'src/' (método manual)...");
  copyDirRecursiveSync('src', path.join(dist, 'src'));
  
  console.log("Copiando 'imagens/' (método manual)...");
  ...
  console.log('-------------------------------------------');
  console.log('✅ SUCESSO! Build gerado em dist/');
  console.log('-------------------------------------------');
